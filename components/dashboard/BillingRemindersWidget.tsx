...
const BillingRemindersWidget: React.FC<BillingRemindersWidgetProps> = ({ reminders }) => {
  return (
    <DashboardCard title="Billing Reminders" className="lg:col-span-2" titleIcon={<AlertTriangleIcon className="w-5 h-5 text-yellow-400" />}>
      {reminders.length === 0 ? (
        ...
        <ul className="space-y-2 max-h-60 overflow-y-auto chat-scrollbar pr-2">
          {reminders.map((reminder) => (
            <li key={reminder.id} className="p-2 bg-[#2c5045] rounded-md flex items-start">
              <AlertTriangleIcon className="w-4 h-4 text-yellow-400 mr-2 mt-0.5 flex-shrink-0" />
              <div>
